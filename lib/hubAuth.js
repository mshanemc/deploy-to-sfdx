...
module.exports = async function () {
	...
	if (process.env.LOCAL_ONLY_KEY_PATH) {
		...
		fs.writeFile('/app/tmp/server.key', process.env.JWTKEY, 'utf8');
	...
	try {
		...
		const writeFileErr = await writeFile('.config/sfdx/unsignedPluginWhiteList.json', JSON.stringify(plugins));
		if (writeFileErr){
