		...
		const properName = `${pool.user}.${pool.repo}`;
			...
			if (!poolTracker[properName]){
				...
				poolTracker[properName] = {
					actual : null,
					enqueuedByMe : 0,
					millsLife: pool.lifeHours*60*60*1000
				};
			}
	});
	return pools;
...
const checkExpiration = async (poolname) => {
	...
	return new Promise((resolve, reject) => {
		...
		.then((poolOrg) => {
			...
			if (moment().diff(moment(msgJSON.createdDate)) > poolTracker[poolname].millsLife) {
...
const prepareAll = async () => {
	...
	_.forEach(pools, async (pool) => {
		let result = await preparePoolByName(pool);
		logger.debug(result);
	});
};
...
const skimmer = async () => {
	const pools = await getPools();
	_.forEach(pools, async (pool) => {
		const result = await checkExpiration(`${pool.user}.${pool.repo}`);
		logger.debug(result);
	});
};
