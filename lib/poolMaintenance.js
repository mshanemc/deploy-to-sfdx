...
const preparePoolByName = async (pool) => {
	...
	if (actualQuantity + inProgress < targetQuantity) {
		...
		return `requesting 1 more org for ${poolname}`;
	} else {
...
const skimmer = async () => {
	// resyncing the poolDeploys/enqueuedByMe that get out of sync sometimes
	const poolDeployLength = await redis.llen('poolDeploys');
	...
	_.forEach(pools, async (pool) => {
		...
		if (poolDeployLength === 0) {
			poolTracker[`${pool.user}.${pool.repo}`].enqueuedByMe = 0;
		}
	});
