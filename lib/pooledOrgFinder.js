			...
			.then((q) => {
				...
				ch.consume(q.queue, (poolMsg) => {
					...
					if (poolMsg && poolMsg.content){ // sanity check for crap messages?
						...
						.then(() => ch.publish(deployMsgExchange, '', bufferKey(openResult.stdout, deployReq.deployId)))
						.then(() => ch.ack(poolMsg))
						.then(() => ch.publish(deployMsgExchange, '', bufferKey('ALLDONE', deployReq.deployId)))
						.then(() => ch.close())
						.then(() => mqConn.close())
						.then(() => {
							resolve(true);
