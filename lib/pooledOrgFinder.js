		...
		return new Promise((resolve) => {
			...
			utilities.getKey(deployReq)
				...
				.then((result) => {
					...
					return redis.publish(deployMsgChannel, bufferKey(openResult.stdout, deployReq.deployId));
				})
				.then(() => {
					return redis.publish(deployMsgChannel, bufferKey('ALLDONE', deployReq.deployId));
				})
				.then(() => {
					redis.disconnect();
					resolve(true);
