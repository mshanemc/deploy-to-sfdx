	...
	this.runLines = async function runLines() {
		...
		for (let line of this.lines) {
			...
			if (localLine.includes('sfdx shane:heroku:repo:deploy')) {
				...
				// adjust the existing line to put in the app name to the current folder/deployId and json it
				localLine = argStripper(localLine, '--json', true);
				localLine = argStripper(localLine, '-n');
				localLine = argStripper(localLine, '--name');
				const appname = msgJSON.deployId.toLowerCase();
				localLine = `${localLine} --name ${appname} --json`;
				...
				const days = utilities.getArg('-d') || utilities.getArg('--days') || 7;

				const herokuDeleteMessage = {
					herokuDelete: true,
					appName: appname,
					expiration: new Date.now() + (days * 24 * 60 * 60 * 1000)
				};
