...
module.exports = function (ch) {
	...
	ch.consume('deploys', (msg) => {
		...
		if (process.env[`${msgJSON.username}/${msgJSON.repo}`] && parseInt(process.env[`${msgJSON.username}/${msgJSON.repo}`] > 0)) {



			// connect to that prebuild topic and see if there's an org
			// verify org isn't expired, else exit and build one (and start replenishing our pool)
			// verify org is "current" with github, else exit and build one (and start replenishing our pool)
			// found an org?  Update the email address of the primary user, then call "open" on it to get a login url and send that message onto the deployMessages queue
		} else {
