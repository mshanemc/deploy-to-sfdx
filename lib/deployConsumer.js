			...
			if (!msg){
				...
				pooledOrgFinder(msgJSON)
					...
					.then((result) => {
						...
						redisPub.publish(ex, bufferKey(result.stderr, msgJSON.deployId));
						return true;
					})
					.then(() => {
					...
					.then((result) => {
						...
						if (!fs.existsSync(`tmp/${msgJSON.deployId}/orgInit.sh`)) {
							...
							return lineParse(msgJSON, redisPub, visitor);
						}
					})
					.then((parsedLines) => {
						...
						if (!parsedLines) {
							...
							let localLineRunner = new lineRunner(msgJSON, parsedLines, redisPub, visitor);
							return localLineRunner.runLines();
						}
					})
					.then(() => {
						// this is true whether we errored or not
						redisPub.publish(ex, bufferKey('ALLDONE', msgJSON.deployId));
