    ...
    switch (command) {
      ...
      case 'create': {
        ...
        commands.run(command, script, () => {
          ...
          script = `${startingDirectory}cd ${directory};export FORCE_SHOW_SPINNER=;sfdx force:org:create -d 1 -v '${access_token}' -s -f ${param}` ;
      ...
      case 'apex': {
        script = `${startingDirectory}cd ${directory};export FORCE_SHOW_SPINNER=;sfdx force:apex:execute -f ${param}`;
        commands.run(command, script, (commandResult, commandErr) => {
      ...
      case 'package': {
        ...
        commands.run(command, script, (commandResult, commandErr) => {
          ...
          if (commandErr) {
            ...
            res.json({
              message: `Installed Package ${param}.`
            });
          }
      ...
      case 'data': {
        script = `${startingDirectory}cd ${directory};export FORCE_SHOW_SPINNER=;sfdx force:data:tree:import -p ${param}`;
      ...
      case 'password':
        // https://hosted-scratch-qa.herokuapp.com/hostedOneClick?template=https://github.com/mshanemc/DF17integrationWorkshops
        script = `${startingDirectory}cd ${directory};export FORCE_SHOW_SPINNER=;sfdx force:org:display --json | jq '.result.accessToken'`;
        commands.run(command, script, (commandResult, commandErr) => {
          if (commandResult){
            commandResult = commandResult.replace(/['"]+/g, '');
            console.log(`scratch org access token is ${commandResult}`);
            access_tokenScratch = commandResult;

            // ok, now try with that token
            script = `${startingDirectory}cd ${directory};export FORCE_SHOW_SPINNER=;sfdx force:config:set instanceUrl=${instance_url};sfdx force:user:password:generate -v '${access_token}' -u '${access_tokenScratch}'`;
            commands.run(command, script, (commandResult2, commandErr2) => {

              if (commandErr2) {
                console.log(commandErr2);
                res.status(500);
                res.json({
                  message: `Error: ${commandErr2}.`
                });
              } else {
                console.log(commandResult2);
                res.json({
                  message: `Your password is ${commandResult2}.`
                });
              }
            });
