...
describe('argStripperTest', () => {
	...
	test(
        'handles backtick for bash characters with more string after the closing tick',
        () => {
	...
	test('handles double quoted strings with lots of spaces', () => {
		...
		expect(argStripper(cmd2, '-g', false)).toBe(
            'sfdx shane:heroku:repo:deploy -r ducati-demo-server -t autodeployed-demos'
        );
	});
